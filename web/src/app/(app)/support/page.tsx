...
/**
 * `My support students` (V1-9, `D-71`/`D-77`/`D-87`).
 *
...
function Row({ r }: { r: SupportStudentRow }) {
  ...
  return (
    <Link href={`/support/${r.intervention_id}`}
      ...
      <div className="min-w-0 flex-1">
        <p className="text-sm font-medium">
          {r.full_name}
          {r.class_label ? <span className="ml-2 text-xs text-muted-foreground">{r.class_label}</span> : null}
        </p>
...
function SupportInner() {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <PageHeader title="My support students"
        subtitle="Staff-only — a support tier never reaches a parent" />

      <p className="mt-4 rounded-xl border border-border bg-card p-4 text-sm leading-relaxed">
        {data.headline}
      </p>
      ...
      {data.groups.map((g) => (
        <section key={g.subject_name} className="mt-5">
          ...
          <div className="space-y-2">
            {g.rows.map((r) => <Row key={r.intervention_id} r={r} />)}
          </div>
        </section>
      ))}
