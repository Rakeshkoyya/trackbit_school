...
def create_year(body: YearCreate, m: CurrentMember = Depends(require_coordinator_up),
...
def activate_year(year_id: uuid.UUID, m: CurrentMember = Depends(require_coordinator_up),
                  db: Session = Depends(get_db)):
    return AcademicService(db).activate_year(m, year_id)


@router.delete("/years/{year_id}", response_model=MessageResponse)
def delete_year(year_id: uuid.UUID, m: CurrentMember = Depends(require_coordinator_up),
                db: Session = Depends(get_db)):
...
def create_term(body: TermCreate, m: CurrentMember = Depends(require_coordinator_up),
                db: Session = Depends(get_db)):
...
def delete_term(term_id: uuid.UUID, m: CurrentMember = Depends(require_coordinator_up),
                db: Session = Depends(get_db)):
...
def create_subject(body: SubjectCreate, m: CurrentMember = Depends(require_coordinator_up),
                   db: Session = Depends(get_db)):
    return AcademicService(db).create_subject(m, body)


@router.delete("/subjects/{subject_id}", response_model=MessageResponse)
def delete_subject(subject_id: uuid.UUID, m: CurrentMember = Depends(require_coordinator_up),
                   db: Session = Depends(get_db)):
...
def create_class(body: ClassCreate, m: CurrentMember = Depends(require_coordinator_up),
                 db: Session = Depends(get_db)):
...
def delete_class(class_id: uuid.UUID, m: CurrentMember = Depends(require_coordinator_up),
                 db: Session = Depends(get_db)):
...
def delete_class_subject(cs_id: uuid.UUID, m: CurrentMember = Depends(require_coordinator_up),
                         db: Session = Depends(get_db)):
